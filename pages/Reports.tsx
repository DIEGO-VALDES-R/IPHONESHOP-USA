...
import { BarChart2, Download, FileSpreadsheet, TrendingUp, Package, Users, MinusCircle, Calendar, RefreshCw, ChevronDown, ArrowUpRight, ArrowDownRight } from 'lucide-react';
...
// ─── Types ────────────────────────────────────────────────────────────────────
type TabId = 'ventas' | 'inventario' | 'cartera' | 'egresos';
...
const groupByDay = (rows: SaleRow[]) => {
  ...
  rows.forEach(r => {
    ...
    map[day] = (map[day] || 0) + r.total_amount;
  });
...
const Reports: React.FC = () => {
  ...
  const loadVentas = useCallback(async () => {
    if (!companyId) return;
    let q = supabase.from('invoices').select('id,invoice_number,created_at,total_amount,subtotal,tax_amount,discount_amount,payment_method,customer_id')
      .eq('company_id', companyId).gte('created_at', fromISO).lte('created_at', toISO).order('created_at', { ascending: false });
  ...
  }, [activeTab, loadVentas, loadInventario, loadCartera, loadEgresos]);
  ...
  const exportExcel = () => {
    ...
    if (activeTab === 'ventas') {
      const rows = sales.map(r => ({
        ...
        'IVA': r.tax_amount, 'Descuento': r.discount_amount || 0,
  ...
  // ── Export PDF (pure jsPDF, no plugins) ──────────────────────────────────
  const exportPDF = () => {
    ...
    const drawTable = (
      ...
      rows.forEach((row, ri) => {
        ...
        colWidths.forEach((w, i) => {
          ...
          doc.text(txt.length > 28 ? txt.slice(0,25)+'…' : txt, x + 2, y + 5.5);
          x += w;
        });
        // row border
      ...
      doc.rect(14, startY, totalW, y - startY);
      return y + 4;
    };
  ...
  const KPICard = ({ label, value, sub, color = 'blue' }: { label: string; value: string; sub?: string; color?: string }) => {
    const colors: Record<string,string> = { blue: 'bg-blue-50 border-blue-100 text-blue-700', green: 'bg-green-50 border-green-100 text-green-700', red: 'bg-red-50 border-red-100 text-red-700', amber: 'bg-amber-50 border-amber-100 text-amber-700', purple: 'bg-purple-50 border-purple-100 text-purple-700' };
    return (
    ...
    <div className="p-4 md:p-6 space-y-5 max-w-7xl mx-auto">
      ...
      <div className="flex flex-wrap gap-2">
        {TABS.map(t => {
          ...
          const colorsActive: Record<string,string> = { blue: 'bg-blue-600 text-white', green: 'bg-emerald-600 text-white', purple: 'bg-purple-600 text-white', red: 'bg-red-600 text-white' };
          return (
                    ...
                    {receivables.map(r => {
                      ...
                      return (
                        <tr key={r.id} className={`hover:bg-slate-50 ${overdue ? 'bg-red-50/30' : ''}`}>
                          ...
                          <td className="px-3 py-2.5">
                            <span className={`px-2 py-0.5 rounded-full text-xs font-bold ${r.status==='PAID'?'bg-green-100 text-green-700':overdue?'bg-red-100 text-red-700':'bg-amber-100 text-amber-700'}`}>{r.status==='PAID'?'Pagado':overdue?'Vencido':'Pendiente'}</span>
                          </td>
                        </tr>
                      );
                    })}
