        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
                    ...
                    <h3 className="text-2xl font-bold text-slate-800 mt-1">{formatMoney(14500000)}</h3>
                </div>
                <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
                    <p className="text-red-500 text-sm font-medium">Vencida > 30 días</p>
                    <h3 className="text-2xl font-bold text-red-600 mt-1">{formatMoney(4500000)}</h3>
                </div>
                <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
                    <p className="text-green-500 text-sm font-medium">Recaudado este mes</p>
                    <h3 className="text-2xl font-bold text-green-600 mt-1">{formatMoney(5000000)}</h3>
                </div>
            </div>
                ...
                <table className="w-full text-left text-sm">
                    <thead className="bg-slate-50 border-b border-slate-200">
                        <tr>
                            ...
                            <th className="px-6 py-4 font-semibold text-slate-700">Monto Orig.</th>
                            <th className="px-6 py-4 font-semibold text-slate-700">Saldo</th>
                            <th className="px-6 py-4 font-semibold text-slate-700">Vencimiento</th>
                            ...
                            <tr key={rec.id} className="hover:bg-slate-50">
                                ...
                                <td className="px-6 py-4">
                                    <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium ${
                                        rec.status === 'OVERDUE' ? 'bg-red-100 text-red-800' : 'bg-yellow-100 text-yellow-800'
                                    }`}>
